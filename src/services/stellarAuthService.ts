...
const challengeStore = new Map<string, { challenge: string; timestamp: number; address: string }>();
...
This signature will not trigger any blockchain transactions.`;
      ...
      Logger.error('Erro ao gerar challenge', error as Error);
      throw error;
    }
  }
  ...
  static async verifySignature(
    ...
    try {
      ...
      Logger.error('Erro na verificação de assinatura', error as Error);
   ...
   */
  static async authenticateWallet(authRequest: AuthRequest): Promise<AuthResponse> {
